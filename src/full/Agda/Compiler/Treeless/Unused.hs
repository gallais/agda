...
usedArguments q t = computeUnused q b (False <$ nhs)
  where (nhs, b) = tLamView t
...
computeUnused q t used = do
  ...
  where
    ...
    goAlt (TACon _ a _ b) = underBinders a <$> go b
...
stripUnusedArguments used t = mkTLams m $ applySubst rho b
  where
    (nhs, b) = tLamView t
    n        = length nhs
    m        = snd <$> filter fst (zip used' nhs)
